-- Fixed Payment Configuration Setup Script
...
-- Step 2: Insert payment method configurations (one by one to avoid syntax issues)
...
VALUES ('stripe', true, false, true, true, false, false, '{"public_key": "pk_test_", "secret_key": "sk_test_"}')
...
INSERT INTO payment_method_config (method_name, enabled, customer_access, partner_access, admin_access, admin_confirmation_required, collect_data_only, config_data) 
VALUES ('paypal', true, true, true, true, true, false, '{"client_id": "paypal_client_id"}')
ON CONFLICT (method_name) DO UPDATE SET 
    enabled = EXCLUDED.enabled,
    customer_access = EXCLUDED.customer_access,
    partner_access = EXCLUDED.partner_access,
...
INSERT INTO payment_method_config (method_name, enabled, customer_access, partner_access, admin_access, admin_confirmation_required, collect_data_only, config_data) 
VALUES ('crypto', true, true, true, true, true, false, '{"wallets": {"BTC": "1FTUbAx5QNTWbxyerMPpxRbwqH3XnvwKQb", "USDT": "TYdFjAfhWL9DjaDBAe5LS7zUjBqpYGkRYB", "ETH": "0xd5fffaa3740af39c265563aec8c14bd08c05e838", "XRP": "rNxp4h8apvRis6mJf9Sh8C6iRxfrDWN7AV", "XRP_TAG": "476565842"}}')
ON CONFLICT (method_name) DO UPDATE SET 
    enabled = EXCLUDED.enabled,
    customer_access = EXCLUDED.customer_access,
    partner_access = EXCLUDED.partner_access,
...
INSERT INTO payment_method_config (method_name, enabled, customer_access, partner_access, admin_access, admin_confirmation_required, collect_data_only, config_data) 
VALUES ('bank', true, true, true, true, true, false, '{"supported_banks": ["Chase", "Bank of America", "Wells Fargo"]}')
ON CONFLICT (method_name) DO UPDATE SET 
    enabled = EXCLUDED.enabled,
    customer_access = EXCLUDED.customer_access,
    partner_access = EXCLUDED.partner_access,
...
SELECT 'payment_method_config' as table_name, COUNT(*) as row_count FROM payment_method_config
UNION ALL
SELECT 'pending_payments' as table_name, COUNT(*) as row_count FROM pending_payments
UNION ALL
SELECT 'wallet_balances' as table_name, COUNT(*) as row_count FROM wallet_balances
UNION ALL
SELECT 'wallet_transactions' as table_name, COUNT(*) as row_count FROM wallet_transactions;

-- Step 5: Show payment method configurations
SELECT method_name, enabled, customer_access, partner_access, admin_access, admin_confirmation_required, collect_data_only
